...
var tiaojian = 0; //这里可以设置  触发条件，比如 自定义的指标函数 发出开仓信号（比如返回了 数值  1 ）, 比如 0 时 是等待，
                  // 等于 2 时 是平仓 也可能 触发了平仓条件
var Amount = 1;   //这里可以把 交易的量（币数）  写在程序里 自动控制（比如，根据盘口量），
                  // 也可以设置在界面上做成界面上的参数，在程序运行的时候传进来。
...
function Calculate (nowAccount, nowDepth) {//计算并更新收益 、 浮动收益 、计算 持仓均价 、持仓量
  ...
  NowPositionInfo.floatProfit = diff_balance + diff_stocks
                                               * nowDepth.Bids[0].Price; //此次交易的浮动盈亏
  Profit                      = (
                                initAccount.Stocks - beginAccount.Stocks
                                ) * nowDepth.Bids[0].Price + (
                                initAccount.Balance - beginAccount.Balance
                                ); //实现盈亏
...
function get_Command () {//负责交互的函数，交互及时更新 相关数值 ，熟悉的用户可以自行扩展
  ...
  if (cmd) {
    ...
    if (arrStr.length === 2) {//接受的不是 数值型的，是按钮型的。
      jsonObjStr = '{' + '"' + arrStr[0] + '"' + ':' + arrStr[1] + '}'; // 把
                                                                        // 字符串数组中的元素重新
                                                                        // 拼接
                                                                        // ，拼接成
                                                                        // JSON
...
function Loop () {//循环主体
  ...
  if (isFirst === true) {
    $.SignOP((
                 new Date()
             ).getTime(), null, null, 3, '图表显示启动！');// 测试标记 自定义信息 到图表上
    Log('程序启动！');
    isFirst = false;
  }
  //--------------------------
  
  //策略运行时状态栏表格上的显示数据 table.b1 就是往 b1 这个格子里 写入 "stock:" + account.Stocks +
  // "#ff00ff"; 这些数据，可以对比截图，自己动手试试。
  ...
  //这里可以自定义  触发 操作的代码，比如 指标交叉了（当然这是你自定义的）， 就可以给 tiaojian 这个变量赋值 1， 即： tiaojian
  // = 1; 这样下面满足条件就执行相应操作。
  ...
  if (state === free && tiaojian === 1) {//开仓条件，可以自行扩展，指标形态、差价、交易量 等等
    ...
    $.SignOP((
                 new Date()
             ).getTime(), buyInfo.price, buyInfo.amount, 1);// 把操作标记 到图表上 .
                                                            // 图表模板 用法可以看论坛上的帖子
    openUpdate();
    ...
    $.SignOP((
                 new Date()
             ).getTime(), sellInfo.price, sellInfo.amount, 0);// 把操作标记 到图表上
    closeUpdate();
  }
...
function main () {
  ...
  table        = $.TableInit(5, 6);
  //初始化表格 5，代表 表格生成5列 分别是 a b c d e    ， 6代表表格生成 6行 分别是  0  1  2  3  4  5  6
  // 。 这样最左上角的表格单元的坐标就是 a0 ,  table.a0 = 3; 此时就会显示到相应的表格
